...
% --------------------------------------------------------   
    ...
    if isempty(strfind(imdbs{1}.name, 'train')) || ~exist([imdbs{1}.details.root_dir '/image_roidb.mat'])
      ...
      load([imdbs{1}.details.root_dir '/image_roidb.mat']);
    end
...
function [image_roidb, means, stds] = append_bbox_regression_targets(conf, image_roidb, means, stds)
    ...
    for i = 1:num_images %should be parfor
...
%          image_roidb(i)=[];
    ...
    for i = 1:num_images
        for j = 1:length(conf.scales)
          if isempty(image_roidb(i).bbox_targets), continue; end
            targets = image_roidb(i).bbox_targets{j};
            gt_inds = find(targets(:, 1) > 0);
            if ~isempty(gt_inds)
...
%   negative: [-1, zero(regression_label)]
    ...
    [regression_label] = rfcn_bbox_transform(src_rois, target_rois);
