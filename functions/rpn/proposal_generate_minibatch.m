...
% --------------------------------------------------------
    ...
    for i = 1:num_images
        ...
        if 0
%           im = fliplr(imread(  image_roidb(i).image_path ));
          im = (imread(  image_roidb(i).image_path ));
          boxes_cell = {bbox_targets(labels>0,:)};
          
          figure('windowstyle','docked'); 
          imd(im);
%           showboxes(im, boxes_cell);
%           drawBBoxes(boxes_cell, 'LineWidth', 1);
%           figure('windowstyle','docked'); imd(uint8(im_blob(:,:,4:6,i)+128))
        end
        % get fcn output size
        img_size = round(image_roidb(i).im_size * im_scales(i));
        img_size = size(im_blob); img_size = img_size(1:2); %fix for different sizes
...
function [im_blob, im_scales] = get_image_blob(conf, images, random_scale_inds)
    ...
    for i = 1:num_images
        ...
        switch conf.modality
          case 'flow_600'
            im_u = imread(strrep(images(i).image_path,'jpegs',['tvl1_flow_600' filesep 'u']));
            im_v = imread(strrep(images(i).image_path,'jpegs',['tvl1_flow_600' filesep 'v']));
%             g = imread(strrep(images(i).image_path,'jpegs',['jpegs_600']));
            
            im = single(cat(3,im_u,im_v));
%             flow = (flow - 128) .* (40 / 256) ;

            mag_flow = sqrt(sum(im.^2,3));
                
                im = cat(3,im,mag_flow);
                im = uint8(im);
          case 'flow_hsv'
            im = imread(strrep(images(i).image_path,'jpegs',['hsvflow_600']));
          otherwise
