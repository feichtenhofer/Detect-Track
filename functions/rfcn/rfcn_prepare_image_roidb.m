...
% --------------------------------------------------------   
    ...
    imdbs = imdbs(:);
    roidbs = roidbs(:);
    imdbs_name = cellfun(@(x) x.name, imdbs, 'UniformOutput', false);

    fn = [imdbs{1}.details.root_dir '/image_roidb_rpn_' [imdbs_name{:}] '.mat'];
    if ~exist(fn)
    ...
    save(fn, 'image_roidb', 'bbox_means', 'bbox_stds' ,'-v7.3')
